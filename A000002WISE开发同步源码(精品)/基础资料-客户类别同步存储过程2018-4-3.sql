alter procedure pro_HT_CustGroupSync

as
begin
set nocount on
--1.更新客户类别
--1.1找出（非禁用）客户类别的辅助表分组数据,插入新帐套
...
and t1.FDeleted=0
and not exists 
(select * from t_HT_SyncControl where FOID=t1.FItemID and FType='辅助表-客户类别')
...
'辅助表-客户类别',1,0,null,null,
@FOItemID,null,null,null

select @FItemID=FItemID from t_item where FItemClassID=1 and FNumber=@FNumber
...
Delete from Access_t_Organization where FItemID=@FItemID
...
update t_Item set FName=FName where FItemID=@FItemID and FItemClassID=1

UPDATE t_Organization SET FRegUserID=16394,FRegDate=GETDATE() WHERE FItemID=@FItemID

UPDATE t1 SET t1.FRegDate = t2.FCreateDate FROM t_Organization t1 
INNER JOIN t_BaseProperty t2 ON t1.FItemID = t2.FItemID
WHERE t1.FItemID=@FItemID AND t2.FTypeID=3
...
from #tmp2 t1
left join 
(select * from t_item where FItemClassID=1 and FDetail=0 and FDeleted=0) t2 on t1.FParentNumber=t2.FNumber
...
where t1.FItemClassID=1 and t1.FDetail=0
...
--exec pro_HT_CustGroupSync
--delete from t_item where fitemclassid=1 and fdetail=0
--delete from t_HT_SyncControl where FType='辅助表-客户类别'