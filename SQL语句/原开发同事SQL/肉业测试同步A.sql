-- 源账套 188.188.1.11(服务器)  Test_YXRY2_A  (数据库)
---目账套 188.188.1.4 (服务器)  Test_YXRY2_B(数据库)
...
select 'CON14.Test_YXRY2_B','帐套同步'
...
ISNULL(FOtherARAcctID,0),FParentID,FPersonalBank,FPhone,ISNULL(FPreAPAcctID,0),ISNULL(FPreARAcctID,0),ISNULL(FProfessionalGroup,0),FShortNumber FROM Inserted  
 ...
 ------4 t_icitem 触发器
 if exists(select * from dbo.sysobjects  where id = object_id(N'[dbo].[TR_t_ICItem]')  and OBJECTPROPERTY(id, N'IsTrigger') = 1)
PRINT '1' ELSE PRINT '0'
...
-- Description:	部门同步
-- =============================================
 if exists(select * from dbo.sysobjects  where id = object_id(N'[dbo].[yj_t_Department]')  and OBJECTPROPERTY(id, N'IsTrigger') = 1)
...
begin
	...
	begin
------*********************** 此处要改目标帐套名*****************************************	
	  IF EXISTS(select 1 from CON14.Test_YXRY2_B.dbo..890 where isnull(FCheckerID,0)>0 and FInterID= @FSynchroID)
------*********************** 此处要改目标帐套名*****************************************
  begin
...
--SELECT TOP 1 @FCheckerID=isnull(FProcessUserID,0)@FCheckerID=isnull(FChecker,0),  FROM ICClassMCTaskCenter where FClassTypeID = @FClassTypeID  AND fbillid=@FInterID ORDER BY fid desc
  ...
  --      set @BNoSql =' exec '+ @DBName+'.dbo.Pro_BosBillNo  ''XSKD'',40020,@p2 output'
	...
	begin
--**************************************************
		  IF EXISTS(select 1 from CON14.Test_YXRY2_B.dbo.t_RP_NewReceiveBill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
--**************************************************
...
DECLARE itemdetail_cursor CURSOR FOR
 --********************************************************
    SELECT a.* FROM   (SELECT ISNULL((SELECT TOP 1 Fitemid FROM CON14.Test_YXRY2_B.dbo.t_item WHERE  FItemClassID=1 AND  FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE  FItemClassID=1 AND  FItemID=f1),0)),0) AS F1,
  ISNULL((SELECT TOP 1 Fitemid FROM CON14.Test_YXRY2_B.dbo.t_item WHERE FItemClassID=2 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=2 AND FItemID=f2),0)),0) AS F2,
  ISNULL((select TOP 1 Fitemid FROM CON14.Test_YXRY2_B.dbo.t_item WHERE FItemClassID=3 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=3 AND FItemID=f3),0)),0) AS F3,
  ISNULL((select TOP 1 Fitemid FROM CON14.Test_YXRY2_B.dbo.t_item WHERE FItemClassID=8 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=8 AND FItemID=f8),0)),0) AS F8,
  ISNULL((select TOP 1 Fitemid FROM CON14.Test_YXRY2_B.dbo.t_item WHERE FItemClassID=3001 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=3001 AND FItemID=F3001),0)),0) AS F3001,
  a.FdetailCount 
  --****************************************************************
   FROM t_itemdetail a,t_VoucherEntry b WHERE a.FDetailID=b.FDetailID AND b.FVoucherID=@FinterID AND a.FDetailID>0 ) a ,CON14.Test_YXRY2_B.dbo.t_itemdetail b 
...
BEGIN
--****************************************************
SELECT @FdetailIDMax=ISNULL(MAX(FdetailID)+1,0) FROM CON14.Test_YXRY2_B.dbo.t_itemdetail
INSERT INTO CON14.Test_YXRY2_B.dbo.T_itemdetail (F1,F2,F3,F8,F3001,FdetailCount,FdetailID)
...
IF @F1>0
     ...
     --*****************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.Test_YXRY2_B.dbo.t_Item WHERE  FItemClassID=1 AND  FItemID=@F1),0),ISNULL(@F1,0)  from CON14.Test_YXRY2_B.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F2>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
     --********************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.Test_YXRY2_B.dbo.t_Item WHERE FItemClassID=2 AND FItemID=@F2),0),ISNULL(@F2,0)  from CON14.Test_YXRY2_B.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F3>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.Test_YXRY2_B.dbo.t_Item WHERE FItemClassID=3 AND FItemID=@F3),0),ISNULL(@F3,0)  from CON14.Test_YXRY2_B.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F8>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.Test_YXRY2_B.dbo.t_Item WHERE FItemClassID=8 AND FItemID=@F8),0),ISNULL(@F8,0)  from CON14.Test_YXRY2_B.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	  ---20150422yxf add
	  IF @F3001>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.Test_YXRY2_B.dbo.t_Item WHERE FItemClassID=3001 AND FItemID=@F3001),0),ISNULL(@F3001,0)  from CON14.Test_YXRY2_B.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
...
--**************************************
UPDATE a SET FNext=FNext+1 FROM CON14.Test_YXRY2_B.dbo.t_Identity a WHERE FName='t_Voucher'
SELECT TOP 1 @pp=FNext FROM CON14.Test_YXRY2_B.dbo.t_Identity WHERE FName='t_Voucher'
...
--*********************************************************
					  IF EXISTS(select 1 from CON14.Test_YXRY2_B.dbo.t_rp_arpbill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
