...
DELETE FROM #T_YX_SubInStock WHERE FBillNo IN(SELECT FBillNo FROM T_YX_SubInStock WHERE FDate >='2018-06-01' ) 
...
SELECT  @CountNum=COUNT(*) FROM #T_YX_SubInStock	    
...
SELECT @FID=FID FROM #T_YX_SubInStock t WHERE FZID=@ZID

INSERT INTO T_YX_SubInStock
SELECT FID,FClassTypeID,FBillNo,FDate,
...
FCheckDate,
FChecker=(select FUserID from t_User WHERE FName=(SELECT FName  FROM CON11.AIS_YXSP2.dbo.t_User WHERE FUserID=t.FChecker  ))  
FROM #T_YX_SubInStock t  WHERE fzid=@ZID
...
Fqty,Fprice,FAmount
FROM CON11.AIS_YXSP2.dbo.T_YX_SubInStockEntry t where FID= @FID


--插入记录表 ICClassCheckRecords200000012
DELETE FROM ICClassCheckRecords200000012 WHERE FBillID=@FID
...
FROM CON11.AIS_YXSP2.dbo.ICClassCheckRecords200000012 t WHERE FBillID=@FID


--插入审核表 ICClassCheckStatus200000012
DELETE FROM ICClassCheckStatus200000012 WHERE FBillID=@FID
INSERT ICClassCheckStatus200000012(FPage,FBillID,FBillEntryID,FBillNo,FBillEntryIndex,FCurrentLevel,FCheckMan1,FCheckIdea1,FCheckDate1)
...
FCheckIdea1,FCheckDate1 FROM CON11.AIS_YXSP2.dbo.ICClassCheckStatus200000012 t WHERE FBillID= @FID
