...
begin
	...
	begin
	   ...
	   begin
			set @Sql = 'insert into ' + @DBName + '.dbo.ICStockBill(FBrNo,FInterID,FTranType,FDate,FBillNo,FRefType,
				...
				isnull((select top 1 FItemID from ' + @DBName+'.dbo.t_item where FNumber=(select FNumber from t_item where FItemID=t.FSupplyID)),0),
	...
	begin
		...
		set @Sql ='delete from ' + @DBName + '.dbo.ICStockBill where FInterID=' + @FSynchroID
		exec(@sql) 
