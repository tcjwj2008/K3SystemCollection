...
begin
	...
	begin
------*********************** 此处要改目标帐套名*****************************************	
	  IF EXISTS(select 1 from AIS_yxry2_2.dbo.ICStockBill where isnull(FCheckerID,0)>0 and FInterID= @FSynchroID)
------*********************** 此处要改目标帐套名*****************************************
  begin
...
--SELECT TOP 1 @FCheckerID=isnull(FProcessUserID,0)@FCheckerID=isnull(FChecker,0),  FROM ICClassMCTaskCenter where FClassTypeID = @FClassTypeID  AND fbillid=@FInterID ORDER BY fid desc
  ...
  --      set @BNoSql =' exec '+ @DBName+'.dbo.Pro_BosBillNo  ''XSKD'',40020,@p2 output'
	...
	begin
--**************************************************
		  IF EXISTS(select 1 from AIS_yxry2_2.dbo.t_RP_NewReceiveBill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
--**************************************************
...
DECLARE itemdetail_cursor CURSOR FOR
 --********************************************************
    SELECT a.* FROM   (SELECT ISNULL((SELECT TOP 1 Fitemid FROM AIS_yxry2_2.dbo.t_item WHERE  FItemClassID=1 AND  FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE  FItemClassID=1 AND  FItemID=f1),0)),0) AS F1,
  ISNULL((SELECT TOP 1 Fitemid FROM AIS_yxry2_2.dbo.t_item WHERE FItemClassID=2 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=2 AND FItemID=f2),0)),0) AS F2,
  ISNULL((select TOP 1 Fitemid FROM AIS_yxry2_2.dbo.t_item WHERE FItemClassID=3 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=3 AND FItemID=f3),0)),0) AS F3,
  ISNULL((select TOP 1 Fitemid FROM AIS_yxry2_2.dbo.t_item WHERE FItemClassID=8 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=8 AND FItemID=f8),0)),0) AS F8,
  a.FdetailCount 
  --****************************************************************
   FROM t_itemdetail a,t_VoucherEntry b WHERE a.FDetailID=b.FDetailID AND b.FVoucherID=@FinterID AND a.FDetailID>0 ) a ,AIS_yxry2_2.dbo.t_itemdetail b 
...
BEGIN
--****************************************************
SELECT @FdetailIDMax=ISNULL(MAX(FdetailID)+1,0) FROM AIS_yxry2_2.dbo.t_itemdetail
INSERT INTO AIS_yxry2_2.dbo.T_itemdetail (F1,F2,F3,F8,FdetailCount,FdetailID)
...
IF @F1>0
     ...
     --*****************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM AIS_yxry2_2.dbo.t_Item WHERE  FItemClassID=1 AND  FItemID=@F1),0),ISNULL(@F1,0)  from AIS_yxry2_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F2>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
     --********************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM AIS_yxry2_2.dbo.t_Item WHERE FItemClassID=2 AND FItemID=@F2),0),ISNULL(@F2,0)  from AIS_yxry2_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F3>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM AIS_yxry2_2.dbo.t_Item WHERE FItemClassID=3 AND FItemID=@F3),0),ISNULL(@F3,0)  from AIS_yxry2_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F8>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM AIS_yxry2_2.dbo.t_Item WHERE FItemClassID=8 AND FItemID=@F8),0),ISNULL(@F8,0)  from AIS_yxry2_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	--************************************************************* 
    FETCH NEXT FROM itemdetail_cursor into @F1,@F2,@F3,@F8,@FdetailCount 
END
...
--**************************************
UPDATE a SET FNext=FNext+1 FROM AIS_yxry2_2.dbo.t_Identity a WHERE FName='t_Voucher'
SELECT TOP 1 @pp=FNext FROM AIS_yxry2_2.dbo.t_Identity WHERE FName='t_Voucher'
...
--*********************************************************
					  IF EXISTS(select 1 from AIS_yxry2_2.dbo.t_rp_arpbill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
