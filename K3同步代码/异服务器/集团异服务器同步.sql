...
--备份系统原有的触发器
   ...
   CREATE TRIGGER TR_t_ICItem ON t_ICItem        
        ...
        FPlanPriceMethod,FPlanClass,FPY,FPinYin)  SELECT FItemID,FModel,FName,FHelpCode,ISNULL(FDeleted,0),FShortNumber,FNumber,ISNULL(FParentID,0),ISNULL(FBrNo,'0'),ISNULL(FTopID,0),FRP,FOmortize,FOmortizeScale,ISNULL(FForSale,0),FStaCost,FOrderPrice,FOr
derMethod,  
        FPriceFixingType,FSalePriceFixingType,ISNULL(FPerWastage,0),FARAcctID,FPlanPriceMethod,FPlanClass,ISNULL(FPY,' '),ISNULL(FPinYin,' ') FROM Inserted     INSERT INTO t_ICItemBase ( FItemID,FErpClsID,FUnitID,FUnitGroupID,FDefaultLoc,FSPID,FSource,FQt
yDecimal,  
        FLowLimit,FHighLimit,FSecInv,FUseState,FIsEquipment,FEquipmentNum,FIsSparePart,FFullName,FSecUnitID,FSecCoefficient,FSecUnitDecimal,FAlias,FOrderUnitID,FSaleUnitID,FStoreUnitID,FProductUnitID,FApproveNo,FAuxClassID,FTypeID,FPreDeadLine,FSerialClas
sID,  
...
,  
FProfitRate,FSalePrice,FBatchManager,FISKFPeriod,FKFPeriod,FTrack,FPlanPrice,FPriceDecimal,FAcctID,FSaleAcctID,FCostAcctID,FAPAcctID,FGoodSpec,FCostProject,FIsSnManage,FStockTime,FBookPlan,FBeforeExpire,FTaxRate,FAdminAcctID,FNote,FIsSpecialTax,FSOHighLim
it,  
...
FCharSourceItemID,FPlanMode,FCtrlType,FCtrlStraregy,FContainerName,FKanBanCapability,FIsBackFlush,FBackFlushStockID,FBackFlushSPID,FBatchSplitDays,FBatchSplit,FIsFixedReOrder)  SELECT FItemID,isnull(FPlanTrategy,321),isnull(FOrderTrategy,331),isnull(FLead
Time,0)  
...
erted       
INSERT INTO t_ICItemDesign ( FItemID,FChartNumber,FIsKeyItem,FMaund,FGrossWeight,FNetWeight,FCubicMeasure,FLength,FWidth,FHeight,FSize,FVersion,FStartService,FMakeFile,FIsFix,FTtermOfService,FTtermOfUsefulTime)  SELECT FItemID,FChartNumber,isnull(FIsKeyIt
em,0)  
...
isnull(FTtermOfUsefulTime,0) FROM Inserted     INSERT INTO t_ICItemStandard ( FItemID,FStandardCost,FStandardManHour,FStdPayRate,FChgFeeRate,FStdFixFeeRate,FOutMachFee,FPieceRate,FStdBatchQty,FPOVAcctID,FPIVAcctID,FMCVAcctID,FPCVAcctID,FSLAcctID,FCAVAcctI
D,  
...
FInspectionProject,FIsListControl,FProChkMde,FWWChkMde,FSOChkMde,FWthDrwChkMde,FStkChkMde,FOtherChkMde,FStkChkPrd,FStkChkAlrm,FIdentifier,FSampStdCritical,FSampStdStrict,FSampStdSlight)  SELECT FItemID,isnull(FInspectionLevel,352),isnull(FInspectionProjec
t,0),  
...
begin
	...
	begin
------*********************** 此处要改目标帐套名*****************************************	
	  IF EXISTS(select 1 from CON14.AIS_YXJT_2.dbo.ICStockBill where isnull(FCheckerID,0)>0 and FInterID= @FSynchroID)
------*********************** 此处要改目标帐套名*****************************************
  begin
...
--SELECT TOP 1 @FCheckerID=isnull(FProcessUserID,0)@FCheckerID=isnull(FChecker,0),  FROM ICClassMCTaskCenter where FClassTypeID = @FClassTypeID  AND fbillid=@FInterID ORDER BY fid desc
  ...
  --      set @BNoSql =' exec '+ @DBName+'.dbo.Pro_BosBillNo  ''XSKD'',40020,@p2 output'
	...
	begin
--**************************************************
		  IF EXISTS(select 1 from CON14.AIS_YXJT_2.dbo.t_RP_NewReceiveBill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
--**************************************************
...
DECLARE itemdetail_cursor CURSOR FOR
 --********************************************************
    SELECT a.* FROM   (SELECT ISNULL((SELECT TOP 1 Fitemid FROM CON14.AIS_YXJT_2.dbo.t_item WHERE  FItemClassID=1 AND  FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE  FItemClassID=1 AND  FItemID=f1),0)),0) AS F1,
  ISNULL((SELECT TOP 1 Fitemid FROM CON14.AIS_YXJT_2.dbo.t_item WHERE FItemClassID=2 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=2 AND FItemID=f2),0)),0) AS F2,
  ISNULL((select TOP 1 Fitemid FROM CON14.AIS_YXJT_2.dbo.t_item WHERE FItemClassID=3 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=3 AND FItemID=f3),0)),0) AS F3,
  ISNULL((select TOP 1 Fitemid FROM CON14.AIS_YXJT_2.dbo.t_item WHERE FItemClassID=8 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=8 AND FItemID=f8),0)),0) AS F8,
  a.FdetailCount 
  --****************************************************************
   FROM t_itemdetail a,t_VoucherEntry b WHERE a.FDetailID=b.FDetailID AND b.FVoucherID=@FinterID AND a.FDetailID>0 ) a ,CON14.AIS_YXJT_2.dbo.t_itemdetail b 
...
BEGIN
--****************************************************
SELECT @FdetailIDMax=ISNULL(MAX(FdetailID)+1,0) FROM CON14.AIS_YXJT_2.dbo.t_itemdetail
INSERT INTO CON14.AIS_YXJT_2.dbo.T_itemdetail (F1,F2,F3,F8,FdetailCount,FdetailID)
...
IF @F1>0
     ...
     --*****************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXJT_2.dbo.t_Item WHERE  FItemClassID=1 AND  FItemID=@F1),0),ISNULL(@F1,0)  from CON14.AIS_YXJT_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F2>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
     --********************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXJT_2.dbo.t_Item WHERE FItemClassID=2 AND FItemID=@F2),0),ISNULL(@F2,0)  from CON14.AIS_YXJT_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F3>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXJT_2.dbo.t_Item WHERE FItemClassID=3 AND FItemID=@F3),0),ISNULL(@F3,0)  from CON14.AIS_YXJT_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F8>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXJT_2.dbo.t_Item WHERE FItemClassID=8 AND FItemID=@F8),0),ISNULL(@F8,0)  from CON14.AIS_YXJT_2.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	--************************************************************* 
    FETCH NEXT FROM itemdetail_cursor into @F1,@F2,@F3,@F8,@FdetailCount 
END
...
--**************************************
UPDATE a SET FNext=FNext+1 FROM CON14.AIS_YXJT_2.dbo.t_Identity a WHERE FName='t_Voucher'
SELECT TOP 1 @pp=FNext FROM CON14.AIS_YXJT_2.dbo.t_Identity WHERE FName='t_Voucher'
...
--*********************************************************
					  IF EXISTS(select 1 from CON14.AIS_YXJT_2.dbo.t_rp_arpbill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
