...
---- 二 添加同步触发器
ALTER   TRIGGER [dbo].[TR_ICExpenses_Update]
            ON [dbo].[ICExpenses]
FOR Update
AS 
...
--       update t_RP_Contact set FStatus=FStatus | 1,FToBal=1  where FRPBillID=1945
	...
	BEGIN
	  ...
	  SET @sql=@sql+' DELETE ' + @DBName +'.dbo.t_RP_Contact WHERE FRPBillID=(SELECT FRecBillInterID FROM ' + @DBName +'.dbo.ICExpenses WHERE finterid='+@FSynchroID+') '
	 ...
	 exec(@sql)
	 UPDATE ICExpenses SET FSynchroID=NULL WHERE FInterID=@FInterID
