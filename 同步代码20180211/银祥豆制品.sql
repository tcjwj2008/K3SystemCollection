-- 源账套 188.188.1.11(服务器)  AIS_YXDZP2018  (数据库)
---目账套 188.188.1.4 (服务器)  AIS_YXDZP2018_3(数据库)
                ...
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN
                        ...
                        SET @nSql = ' select @p2=FItemID from  ' + @DBName
                            + '.dbo.t_item where FItemClassID='
                            + @FItemClassID + ' and FNumber=' + ''''
                            + @FNumber + '''';
                        ...
                        IF @pp2 = 0
                            BEGIN
                                ...
                                SET @Sql = @T_ItemSQL
                                    + 'select FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,isnull((select top 1 FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID= t.FItemClassID and Fnumber=(select FNumber from T_Item where  FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from T_Item t where FItemClassID= '
                                    + '''' + @FItemClassID + ''''
                                    + ' and FNumber=' + '''' + @FNumber + '''';
                                EXEC (@SQL);	
                            END;
						...
						---- 2.修改	----
                                IF @pp2 > 0
                                    BEGIN
                                        ...
                                        SET @FParentID = 0;
                                        SET @nSql = ' select @p2=isnull((select top 1 FItemID as FParentID from '
                                            + @DBName
                                            + '.dbo.T_Item 
								...
								from T_Item t where FItemID=' + @FItemID;
                                        ...
                                        SET @Sql = 'update   ' + @DBName
                                            ...
                                            + @FShortNumber + ''''
                                            + ',FParentID='
                                            + CAST(@FParentID AS VARCHAR(10))
                                            + '  where FItemID='
                                            + CAST(@pp2 AS VARCHAR(10));
                                        EXEC (@Sql);	
					
                                        UPDATE  T_Item
                                        SET     FFullName = FFullName
...
--        BEGIN  
--             INSERT INTO t_Base_Emp (FItemID,FAccountName,FAddress,FAllotPercent,FAllotWeight,FBankAccount,FBankID,FBirthday,FBrNO,FCreditAmount,FCreditDays,FCreditLevel,FCreditPeriod,FDegree,FDeleted,FDepartmentID,FDuty,FEmail,FEmpGroup,FEmpGroupID,FGend
--er,FHireDate,FID,FIsCreditMgr,FItemDepID,FJobTypeID,FLeaveDate,FMobilePhone,FName,FNote,FNumber,FOperationGroup,FOtherAPAcctID,FOtherARAcctID,FParentID,FPersonalBank,FPhone,FPreAPAcctID,FPreARAcctID,FProfessionalGroup,FShortNumber)  
                    ...
                    BEGIN
                        ...
                        IF @pp2 = 0			--插入到t_Emp和t_item
                            BEGIN
                               ...
                               --改为直接插入表
                                SET @Sql = ' INSERT INTO ' + @DBName
                                    + '.dbo.t_Base_Emp (FItemID,FAddress,FAllotPercent,FBankAccount,FBankID,FBirthday,FBrNO,FCreditAmount,
                                     ...
                                     SELECT ' + CAST(@pp2 AS VARCHAR(10))
                                    + ',FAddress,ISNULL(FAllotPercent,0),FBankAccount,ISNULL(FBankID,0),FBirthday,ISNULL(FBrNO,''0''),
                                    ...
                                    + @DBName
                                    + '.dbo.t_department where Fnumber=(select FNumber from t_department where FItemID=t.FDepartmentID)),0)    as  FDepartmentID,
                                ...
                                SET @Sql = @Sql + ' INSERT INTO ' + @DBName
                                    + '.dbo.HR_Base_Emp(FItemID,Name,Sex,Birthday,Mobile,IDCardID,EMail,EM_ID)
                                          SELECT ' + CAST(@pp2 AS VARCHAR(10))
                                    + ',FName,Case FGender When 1068 Then 1 When 1069 Then 2 Else 0 End,FBirthDay,FMobilePhone,FID,FEMail,NEWID() 
                                          FROM t_Emp 
...
--    INSERT INTO t_ICItemCustom(F_101,F_102,F_103,F_104,F_105,FItemID) SELECT  isnull(F_101,0),isnull(F_102,0),F_103,F_104,F_105,FItemID FROM Inserted  
--        END  
--        IF (@@error <> 0)    
--            ROLLBACK TRANSACTION    
--        ELSE    
            ...
            BEGIN    
                ...
                IF LEN(@DBName) > 0
                    BEGIN
                        ...
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.T_Item where FItemClassID=4 and  FNumber='''
                            + @FNumber + '''';
                        EXEC (@sql);
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.t_ICItem where FNumber=''' + @FNumber
                            + '''';
                        EXEC (@sql);
                    END;
            END;
...
----同步添加(开始)    
            ...
            BEGIN    
                ...
                IF LEN(@DBName) > 0
                    BEGIN			
                        ...
                        SET @InsertSQL = 'Insert into ' + @DBName
                            + '.dbo.t_ICItem (FHelpCode,FModel,FErpClsID,FTypeID,'
                        ...
                        SET @FItemidTemp = 0;
                        SET @strSQL = ' select @p2=FItemID from ' + @DBName
                            + '.dbo.T_Item where  FItemClassID=4 and Fnumber='
                            + '''' + @FNumber + '''';
                        EXEC sp_executesql @strSQL, N'@p2   int   output ',
                            @FItemidTemp OUTPUT;
		
                        IF @FItemidTemp = 0
                            BEGIN
                                SET @strSQL = ' exec ' + @DBName
                                    + '.dbo.GetICMaxNum ''T_Item'',@p2 output,1,16394';
		...
		-------
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.T_Item where FItemClassID=4 and FItemID='
                            + @FItemidTemp;
                        EXEC (@sql);

                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.T_Icitem where FItemID=' + @FItemidTemp;
                        EXEC (@sql);
		...
		------	t_Item ----
                        SET @Sql = @T_ItemSQL
                            + 'select FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,isnull((select top 1 FItemID as FParentID from '
                            + @DBName
                            + '.dbo.T_Item where  FItemClassID=4 and Fnumber=(select top 1 FNumber from T_Item where  FItemClassID=4 and FItemID=a.FParentID)),0),'
                            + @FItemidTemp + ' from T_Item a where FITemID='
                            + @FItemID;
                        EXEC (@SQL);	
		
                        SET @Sql = ' update  ' + @DBName
                            + '.dbo.t_Item set FName=FName where FItemID='
                            + CAST(@FItemidTemp AS VARCHAR(10));
                        EXEC (@sql);


				
                        SET @Sql = @InsertSQL
                            + ' select  FHelpCode,FModel,FErpClsID,FTypeID,'
                            + 'isnull((select top 1 FItemClassID as FAuxClassID from  '
                            + @DBName
                            + '.dbo.t_ItemClass where FNumber=(select top 1 FNumber from t_ItemClass where FItemClassID=a.FAuxClassID)),0),'
                            + 'isnull((select top 1 FUnitGroupID from  '
                            + @DBName
                            + '.dbo.t_UnitGroup where FName=(select top 1 FName from t_UnitGroup where FUnitGroupID=a.FUnitGroupID)),0),'
                            + 'isnull((select top 1 FMeasureUnitID as FUnitID from  '
                            + @DBName
                            + '.dbo.t_measureunit where FNumber=(select top 1 FNumber from t_measureunit where FMeasureUnitID=a.FUnitID)),0),'
                            + 'isnull((select top 1 FMeasureUnitID as FOrderUnitID from  '
                            + @DBName
                            + '.dbo.t_measureunit where FNumber=(select top 1 FNumber from t_measureunit where FMeasureUnitID=a.FOrderUnitID)),0),'
                            + 'isnull((select top 1 FMeasureUnitID as FSaleUnitID from  '
                            + @DBName
                            + '.dbo.t_measureunit where FNumber=(select top 1 FNumber from t_measureunit where FMeasureUnitID=a.FSaleUnitID)),0),'
                            + 'isnull((select top 1 FMeasureUnitID as FProductUnitID from  '
                            + @DBName
                            + '.dbo.t_measureunit where FNumber=(select top 1 FNumber from t_measureunit where FMeasureUnitID=a.FProductUnitID)),0),'
                            + 'isnull((select top 1 FMeasureUnitID as FStoreUnitID from  '
                            + @DBName
                            + '.dbo.t_measureunit where FNumber=(select top 1 FNumber from t_measureunit where FMeasureUnitID=a.FStoreUnitID)),0),'
                            + 'isnull((select top 1 FMeasureUnitID as FSecUnitID from  '
                            + @DBName
                            + '.dbo.t_measureunit where FNumber=(select top 1 FNumber from t_measureunit where FMeasureUnitID=a.FSecUnitID)),0),'
                            + 'FSource,FDefaultLoc,'
                            + --'isnull((select  FItemID  from  ' +
			...
			--@DBName+'.dbo.t_stock where FNumber=(select top 1 FNumber from t_stock where FItemID=a.FDefaultReadyLoc)),0),'+
                            'isnull((select FItemID from ' + @DBName
                            + '.dbo.t_emp where FNumber=(select top 1 FNumber from t_emp where FItemID=a.FOrderRector)),0),'
                            + 'isnull((select FAccountID from  ' + @DBName
                            + '.dbo.t_Account where FNumber=(select top 1 FNumber from t_Account where FAccountID=a.FAcctID)),0),'
                            + 'isnull((select FAccountID from  ' + @DBName
                            + '.dbo.t_Account where FNumber=(select top 1 FNumber from t_Account where FAccountID=a.FSaleAcctID)),0),'
                            + 'isnull((select FAccountID from  ' + @DBName
                            + '.dbo.t_Account where FNumber=(select top 1 FNumber from t_Account where FAccountID=a.FCostAcctID)),0),'
                            + 'isnull((select FAccountID from  ' + @DBName
                            + '.dbo.t_Account where FNumber=(select top 1 FNumber from t_Account where FAccountID=a.FAPAcctID)),0),'
                            + 'isnull((select FAccountID from  ' + @DBName
                            + '.dbo.t_Account where FNumber=(select top 1 FNumber from t_Account where FAccountID=a.FAdminAcctID)),0),'
                            + 'FSecCoefficient,FSPID,FQtyDecimal,'
                            + 'FLowLimit,FHighLimit,FSecInv,FUseState,FIsEquipment,FEquipmentNum,FIsSparePart,FFullName,FApproveNo,FAlias,'
                            + 'FPOHighPrice,FPOHghPrcMnyType,FWWHghPrc,FWWHghPrcMnyType,FSOLowPrc,FSOLowPrcMnyType,FIsSale,FProfitRate,FOrderPrice,'
                            + 'FSalePrice,FIsSpecialTax,FISKFPeriod,FKFPeriod,FStockTime,FBatchManager,FBeforeExpire,FCheckCycUnit,FOIHighLimit,'
                            ...
                            + @DBName
                            + '.dbo.T_Item where  FItemClassID=4 and Fnumber=(select top 1 FNumber from T_Item where FItemClassID=4 and FItemID=a.FParentID)),0),'
                            + @FItemidTemp + ' from t_ICItem a where FItemID='
                            + @FItemID;		
			...
			---- 1. 添加 ----    
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN
                        ...
                        SET @InsertSQL = 'Insert into ' + @DBName
                            + '.dbo.t_Department (FPhone,FFax,FDProperty,FIsCreditMgr,FNote,FCostAccountType,FOtherARAcctID,FPreARAcctID,FOtherAPAcctID,FPreAPAcctID,FShortNumber,FNumber,FName,FManager,FAcctID,FParentID,FItemID) '; 
                        SET @T_ItemSQL = 'Insert into ' + @DBName
                            + '.dbo.t_item (FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,FParentID,FItemID) ';
                        ...
                        SET @pp2 = 0;
                        SET @nSql = ' select @p2=FItemID from  ' + @DBName
                            + '.dbo.t_Department where  FNumber=' + ''''
                            + @FNumber + '''';
                        EXEC sp_executesql @nSql, N'@p2   int   output ',
                            @pp2 OUTPUT;
                        ...
                        IF @pp2 = 0
                            BEGIN
                                ...
                                SET @Sql = @T_ItemSQL
                                    ...
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=2 and Fnumber=(select FNumber from T_Item where  FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from T_Item t where FITemID='
                                    + @FItemID;
                                EXEC (@SQL);	
                                ...
                                SET @Sql = @InsertSQL
                                    + ' select FPhone,FFax,FDProperty,FIsCreditMgr,FNote,FCostAccountType,FOtherARAcctID,FPreARAcctID,FOtherAPAcctID,FPreAPAcctID,FShortNumber,FNumber,FName,'
                                    + 'isnull((select FItemID from ' + @DBName
                                    + '.dbo.t_emp where  Fnumber=(select FNumber from t_emp where FItemID=t.FManager)),0),'
                                    + 'isnull((select FAccountID from '
                                    + @DBName
                                    + '.dbo.t_Account where Fnumber=(select FNumber from t_Account where FAccountID=t.FAcctID)),0),'
                                    + 'isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=2 and Fnumber=(select FNumber from T_Item where FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from t_Department t where FItemID='
                                    + @FItemID;	
                                EXEC (@sql);
	
                                SET @Sql = 'update ' + @DBName
                                    + '.dbo.T_Item set FFullName= FName where FItemID ='
                                    + CAST(@pp2 AS VARCHAR(10));
                                EXEC (@sql);
                    ...
                    BEGIN 
                        ...
                        IF @pp2 > 0
                            BEGIN
                                ...
                                SET @Sql = 'delete from ' + @DBName
                                    + '.dbo.T_Item where FItemClassID=2 and FItemID='
                                    + CAST(@pp2 AS VARCHAR(10));
                                EXEC (@sql);

                                SET @Sql = 'delete from ' + @DBName
                                    + '.dbo.t_Department where FItemID='
                                    + CAST(@pp2 AS VARCHAR(10));
                                EXEC (@sql);
                ...
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN 
                        ...
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.T_Item where FItemClassID=2 and  FNumber='''
                            + @FNumber + '''';
                        EXEC (@sql);
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.t_Department where FNumber=''' + @FNumber
                            + '''';
                        EXEC (@sql);
		...
		---- 1. 添加 ----    
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN
                        ...
                        SET @InsertSQL = 'Insert into ' + @DBName
                            + '.dbo.t_Organization (FHelpCode,FShortName,FAddress,FStatus,FRegionID,FTrade,FContact,FPhone,'
                        ...
                        SET @pp2 = 0;
                        SET @nSql = ' select @p2=FItemID from  ' + @DBName
                            + '.dbo.t_Organization where  FNumber=' + ''''
                            + @FNumber + '''';
                        EXEC sp_executesql @nSql, N'@p2   int   output ',
                            @pp2 OUTPUT;
                        ...
                        IF @pp2 = 0
                            BEGIN
                                ...
                                SET @Sql = @T_ItemSQL
                                    + 'select FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=1 and Fnumber=(select FNumber from T_Item where  FItemID='
                                    + @FParentID + ')),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from T_Item where FITemID=' + @FItemID;
                                EXEC (@SQL);	

                                SET @Sql = @InsertSQL
                                    + ' select FHelpCode,FShortName,FAddress,FStatus,FRegionID,FTrade,FContact,FPhone,FMobilePhone,FFax,FPostalCode,FEmail,'
                                    ...
                                    + 'ISNULL((select FAccountID from '
                                    + @DBName
                                    + '.dbo.t_Account where FNumber=(select FNumber from t_Account where FAccountID=t.FARAccountID)),0),
					 ISNULL((select FAccountID from ' + @DBName
                                    + '.dbo.t_Account where FNumber=(select FNumber from t_Account where FAccountID=t.FAPAccountID)),0),
					 ISNULL((select FAccountID from ' + @DBName
                                    + '.dbo.t_Account where FNumber=(select FNumber from t_Account where FAccountID=t.FOtherAPAcctID)),0),
					 ISNULL((select FAccountID from ' + @DBName
                                    + '.dbo.t_Account where FNumber=(select FNumber from t_Account where FAccountID=t.FOtherARAcctID)),0),
					 ISNULL((select FAccountID from ' + @DBName
                                    + '.dbo.t_Account where FNumber=(select FNumber from t_Account where FAccountID=t.FPayTaxAcctID)),0),
					 ISNULL((select FAccountID from ' + @DBName
                                    + '.dbo.t_Account where FNumber=(select FNumber from t_Account where FAccountID=t.FpreAcctID)),0),
					 ISNULL((select FAccountID from ' + @DBName
                                    + '.dbo.t_Account where FNumber=(select FNumber from t_Account where FAccountID=t.FPreAPAcctID)),0),
					'
                                    ...
                                    + 'isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=1 and Fnumber=(select FNumber from T_Item where  FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from t_Organization t where FItemID='
                                    + @FItemID;		
                                EXEC (@sql); 
					
                                SET @Sql = 'IF not exists(Select * from '
                                    + @DBName
                                    + '.dbo.Access_t_Organization where FItemID='
                                    + CAST(@pp2 AS VARCHAR(10)) + ')'
                                    + 'Insert into ' + @DBName
                                    + '.dbo.Access_t_Organization(FItemID,FParentIDX,FDataAccessView,FDataAccessEdit,FDataAccessDelete)
					select ' + CAST(@pp2 AS VARCHAR(10))
                                    + ',isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item 
		...
		-----2.修改 ----
                IF EXISTS ( SELECT  1
                    ...
                    BEGIN 
                        ...
                        IF @pp2 > 0
                            BEGIN 
                                ...
                                SET @Sql = 'delete from ' + @DBName
                                    + '.dbo.T_Item where FItemClassID=1 and FItemID='
                                    + CAST(@pp2 AS VARCHAR(10));
                                EXEC (@sql);

                                SET @Sql = 'delete from ' + @DBName
                                    + '.dbo.t_Organization where FItemID='
                                    + CAST(@pp2 AS VARCHAR(10));
                                EXEC (@sql);
					...
					select ' + CAST(@pp2 AS VARCHAR(10))
                                    ...
                                    + '.dbo.T_Item 
					where FItemClassID=1 and Fnumber=(select FNumber from T_Item where  FItemID=t.FParentID)),0),
					(SELECT  TOP 1  FDataAccessView FROM ' + @DBName
                                    + '.dbo.Access_t_Organization  ORDER BY FItemID DESC  ),
					(SELECT  TOP 1  FDataAccessEdit  FROM ' + @DBName
                                    + '.dbo.Access_t_Organization  ORDER BY FItemID DESC  ),
					(SELECT  TOP 1  FDataAccessDelete  FROM ' + @DBName
                                    + '.dbo.Access_t_Organization  ORDER BY FItemID DESC  )
					from t_item t where FItemID=' + @FItemID;	 
                                EXEC (@sql);	
		...
		-----3. 删除----
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN 
                        ...
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.T_Item where FItemClassID=1 and  FNumber='''
                            + @FNumber + '''';
                        EXEC (@sql);
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.t_Organization where FNumber='''
		...
		-----1. 添加    -----
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN
                        ...
                        SET @pp2 = 0;
                        SET @nSql = ' select @p2=FItemID from  ' + @DBName
                            + '.dbo.t_item where FItemClassID=2001 and FNumber='
                            + '''' + @FNumber + '''';
                        EXEC sp_executesql @nSql, N'@p2   int   output ',
                            @pp2 OUTPUT;
                        ...
                        IF @pp2 = 0
                            BEGIN
                                ...
                                SET @Sql = 'Insert into ' + @DBName
                                    ...
                                    + 'select FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=2001 and Fnumber=(select FNumber from T_Item where  FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from T_Item t where FITemID='
                                    + @FItemID;
                                EXEC (@Sql);	
                                ...
                                SET @Sql = 'insert into ' + @DBName
                                    + '.dbo.CBCostObj (FItemID,FNumber,FName,FStdID,FStdSubID,FParentID,FShortNumber,FUnUsed,FCalculateType,FStdProductID,FBatchNo,FBomID,
						 ...
						 left join ' + @DBName
                                    + '.dbo.t_Item t2 on t2.FNumber=t.FNumber and t2.FItemClassID=4
						 where t.FItemID=' + CAST(@pp2 AS VARCHAR(10));
                                EXEC (@Sql);	
				
                                SET @Sql = 'insert into ' + @DBName
                                    + '.dbo.CB_CostObj_Product(FCostObjID,FProductID,FQuotiety,FIsStand,FIsDeputy)
    ...
    BEGIN    
		...
		-----  1. 添加   ---- 
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN
                        SET @InsertSQL = 'Insert into ' + @DBName
                            + '.dbo.t_stock (FEmpID,FAddress,FPhone,FProperty,FTypeID,FMRPAvail,FIsStockMgr,FSPGroupID,FShortNumber,FNumber,FName,FParentID,FItemID) ';
                        SET @T_ItemSQL = 'Insert into ' + @DBName
                            + '.dbo.t_item (FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,FFullName,FParentID,FItemID) ';
                        ...
                        SET @pp2 = 0;
                        SET @nSQL = ' select @p2=FItemID from  ' + @DBName
                            + '.dbo.t_Stock where Fnumber=''' + @FNumber
                            + '''';
                        EXEC sp_executesql @nSQL, N'@p2   int   output ',
                            @pp2 OUTPUT;
                        ...
                        IF @pp2 = 0
                            BEGIN
                                ...
                                SET @Sql = @T_ItemSQL
                                    + 'select FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,FFullName,isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=t.FItemClassID and Fnumber=(select FNumber from T_Item where FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from T_Item t where FITemID='
                                ...
                                SET @Sql = @InsertSQL + ' select '
                                    + 'isnull((select FItemID from ' + @DBName
                                    + '.dbo.t_emp where Fnumber=(select FNumber from t_emp where FItemID=t.FEmpID)),0),'
                                    + 'FAddress,FPhone,FProperty,FTypeID,FMRPAvail,FIsStockMgr,FSPGroupID,FShortNumber,FNumber,FName,
							isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=5 and Fnumber=(select FNumber from T_Item where FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from t_stock t where FItemID='
                                    + @FItemID;	
                                EXEC (@sql);
		...
		-----  3. 删除 ----
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN
                        ...
                        FROM    deleted;
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.T_Item where FItemClassID=5 and  FNumber='''
                            + @FNumber + '''';
                        EXEC (@sql);
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.t_stock where  FNumber=''' + @FNumber
	...
	---- 1. 添加 ----    
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN
                        ...
                        SET @InsertSQL = 'Insert into ' + @DBName
                            + '.dbo.t_supplier (FHelpCode,FShortName,FAddress,FStatus,FRegionID,FTrade,FContact,FPhone,'
                        ...
                        IF @pp2 = 0
                            BEGIN
                                ...
                                SET @Sql = @T_ItemSQL
                                    + 'select FItemClassID,FLevel,FName,FNumber,FShortNumber,FFullNumber,FDetail,UUID,FDeleted,FFullName,isnull((select FItemID as FParentID from '
                                    + @DBName
                                    + '.dbo.T_Item where FItemClassID=8  and  Fnumber=(select FNumber from T_Item where FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from T_Item t where FITemID='
                                    + @FItemID;
                                EXEC (@Sql);
                                ...
                                SET @Sql = @InsertSQL
                                    + ' select FHelpCode,FShortName,FAddress,FStatus,FRegionID,FTrade,FContact,FPhone,'
                                    + 'FMobilePhone,FFax,FPostalCode,FEmail,FBank,FAccount,FTaxNum,FValueAddRate,FCountry,FProvince,Fcorperate,FDiscount,FTypeID,'
                                    + 'FStockIDAssignee,FBr,FRegmark,FLicence,FCyID,FSetID,FAPAccountID,FPreAcctID,FOtherAPAcctID,FPayTaxAcctID,'
                                    + 'FARAccountID,FPreARAcctID,FOtherARAcctID,FfavorPolicy,Fdepartment,Femployee,FLastTradeDate,FlastTradeAmount,FlastRPAmount,FmaxDealAmount,'
                                    + 'FminForeReceiveRate,FCreditDays,FNameEN,FAddrEn,FCIQCode,FRegion,FManageType,'
                                    ...
                                    + '.dbo.T_Item where FItemClassID=8  and Fnumber=(select FNumber from T_Item where FItemID=t.FParentID)),0),'
                                    + CAST(@pp2 AS VARCHAR(10))
                                    + ' from t_supplier t where FItemID='
                                    + @FItemID;	
	...
	-----3. 删除----
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN 
                        ...
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.T_Item where FItemClassID=8 and  FNumber='''
                            + @FNumber + '''';
                        EXEC (@sql);
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.t_supplier where FNumber=''' + @FNumber
                            + '''';
                        EXEC (@sql);
    ...
    BEGIN    
		...
		-----1. 添加    
			...
			--Create table TongBuTemp (TempID varchar(10))
                        ...
                        SET @Sql = 'insert into  TongBuTemp (TempID) select FitemID from '
                            + @DBName
                            + '.dbo.t_item where FItemClassID=7 and FNumber='
                            + '''' + @FNumber + '''';
                        EXEC (@sql);
                        ...
                        IF @IsHere > 0
                            BEGIN
                                SET @Sql = 'Insert into ' + @DBName
                                    + '.dbo.t_UnitGroup (FUnitGroupID,FName) ';
                                SET @Sql = @Sql + ' select '
                                    + CAST(@IsHere AS VARCHAR(10))
                                    + ',FName from t_UnitGroup where FUnitGroupID='
                                    + @FItemID;	
                                EXEC (@sql);
                            END;
		...
		-----2.修改(只修改名称FName)
			...
			--exec (@sql) 
                        SET @Sql = 'update ' + @DBName
                            + '.dbo.t_UnitGroup set FName=' + ''''
                            + @NewFnumber + '''' + ' where FName=' + ''''
                            + @FNumber + '''';
                        EXEC (@sql); 
		...
		-----3. 删除
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN 
                        ...
                        FROM    deleted;
                        SET @Sql = '  delete from ' + @DBName
                            + '.dbo.T_Item where FItemClassID=7 and  FParentID IN (select FUnitGroupID as FParentID from '
                            + @DBName + '.dbo.t_UnitGroup where FName=' + ''''
                            + @FNumber + '''' + ')';
                        EXEC (@sql);
 			
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.t_UnitGroup where FName=' + ''''
                            + @FNumber + '''';
                        EXEC (@sql);
                ...
                IF EXISTS ( SELECT  *
                    ...
                    BEGIN 
                        ...
                        SET @Sql = 'delete from ' + @DBName
                            + '.dbo.t_item where FItemClassID=7 and  Fnumber='
                            + '''' + @FNumber + '''';
                        SET @Sql = @Sql + 'delete from ' + @DBName
                            + '.dbo.t_measureunit where Fnumber=' + ''''
                            + @FNumber + '''';
                        EXEC (@sql);
...
--六审:	FMultiCheckLevel6
	--select * from ictranstype  Select top 1 * From t_MultiLevelCheck Where FBillType = 1 order by fchecklevel desc
        ...
        SELECT TOP 1
                @LastCheckLevel = ISNULL(FCheckLevel, 0)
		...
		--------物料、单位、仓库---------------
                        SET @msg = '';
                        SET @nSql = ' select @p2=t2.FNumber from  ICStockBillEntry t inner join t_ICItem t2 on t.FItemID=t2.FItemID  left join '
                            + @DBName
                            + '.dbo.t_ICItem t21 on t21.FNumber=t2.FNumber 
					where t21.FItemID is null and  t.FInterID=' + @FInterID;
                        EXEC sp_executesql @nSql,
                            N'@p2   Varchar(50)  output ', @msg OUTPUT;
                        IF LEN(@msg) > 0
                            BEGIN
                                SET @msg = '<物料：' + @msg
                                    + '>,财务帐套不存在该物料,不能审核!';
                        ...
                        SET @nSql = ' select @p2=t2.FNumber from  ICStockBillEntry t inner join t_measureunit t2 on t.FUnitID=t2.FMeasureUnitID  left join '
                            + @DBName
                            + '.dbo.t_measureunit t21 on t21.FNumber=t2.FNumber 
					where t21.FItemID is null and  t.FInterID=' + @FInterID;
                        EXEC sp_executesql @nSql,
                            N'@p2   Varchar(50)  output ', @msg OUTPUT;
                        IF LEN(@msg) > 0
					...
					inner join t_Stock t2 on t2.FItemID=t.FDCStockID  left join '
                            + @DBName
                            + '.dbo.t_Stock t21 on t21.FNumber=t2.FNumber 
					where t21.FItemID is null and  t.FInterID=' + @FInterID;
                        EXEC sp_executesql @nSql,
                            N'@p2   Varchar(50)  output ', @msg OUTPUT;
                        IF LEN(@msg) > 0
		...
		------------------------
                        ...
                        SET @pp = 0;
                        SET @nSql = ' exec ' + @DBName
                            + '.dbo.GetICMaxNum ''ICStockBill'',@p2 output,1,16394';
                        EXEC sp_executesql @nSql, N'@p2   int   output ',
                            @pp OUTPUT;
		...
		--exec sp_executesql  @nSql ,N'@p2   varchar(50)   output ',@FBillNo   output
                        IF @FTranType = 1
                            ...
                            BEGIN
                                SET @Sql = 'insert into ' + @DBName
                                    + '.dbo.ICStockBill(FBrNo,FInterID,FTranType,FDate,FBillNo,FRefType,
						...
						FPurposeID,FBillTypeID,FExplanation,FCheckerID,FCheckDate,FMultiCheckLevel2,FMultiCheckDate2) '
                                    ...
                                    + ',FTranType,FDate,FBillNo,FRefType,
				isnull((select top 1 FItemID from ' + @DBName
                                    + '.dbo.t_supplier where FNumber=(select FNumber from t_supplier where FItemID=t.FSupplyID)),0),
				isnull((select top 1 FItemID from ' + @DBName
                                    + '.dbo.t_department where FNumber=(select FNumber from t_department where FItemID=t.FDeptID)),0),
				isnull((select top 1 FItemID from ' + @DBName
                                    + '.dbo.t_emp where FNumber=(select FNumber from t_emp where FItemID=t.FEmpID)),0),
				isnull((select top 1 FItemID from ' + @DBName
                                    + '.dbo.t_emp where FNumber=(select FNumber from t_emp where FItemID=t.FFManagerID)),0),
				isnull((select top 1 FItemID from ' + @DBName
                                    + '.dbo.t_emp where FNumber=(select FNumber from t_emp where FItemID=t.FSManagerID)),0),
				isnull((select top 1 FItemID from ' + @DBName
                                    + '.dbo.t_emp where FNumber=(select FNumber from t_emp where FItemID=t.FManagerID)),0),
				isnull((select top 1 FAccountID from ' + @DBName
                                    + '.dbo. t_Account where fNumber=(select top 1 FNumber from t_Account where FAccountID=t.FAcctID)),0),
				...
				isnull((select top 1 fuserid from ' + @DBName
                                    + '.dbo.t_User where FName=(select FName from t_User where fuserid=t.FMultiCheckLevel2)),0), (case when '
                                    + CAST(@LastCheckLevel AS VARCHAR)
                                    + ' =3 then FMultiCheckDate2 else null end) as FMultiCheckDate2  
				from ICStockBill t 
				where t.FinterID=' + @FInterID;  
						...
						FPurposeID,FBillTypeID,FExplanation,FCheckerID,FCheckDate,FMultiCheckLevel2,FMultiCheckDate2) '
                                        ...
                                        + ',FTranType,FDate,FBillNo,FRefType,
				isnull((select top 1 FItemID from ' + @DBName
                                        + '.dbo.t_Organization where FNumber=(select FNumber from t_Organization where FItemID=t.FSupplyID)),0),
				isnull((select top 1 FItemID from ' + @DBName
                                        + '.dbo.t_department where FNumber=(select FNumber from t_department where FItemID=t.FDeptID)),0),
				isnull((select top 1 FItemID from ' + @DBName
                                        + '.dbo.t_emp where FNumber=(select FNumber from t_emp where FItemID=t.FEmpID)),0),
		...
		--select FSynchroID from ICStockBill
                        ...
                        SET @Sql = 'insert into ' + @DBName
                            + '.dbo.ICStockBillEntry(FBrNo,FInterID,FEntryID,FBatchNo,FNote,
				...
				FReProduceType,FSourceTranType,FSourceEntryID,FSourceBillNo,FSourceInterId,FOrderEntryID, FOrderBillNo, FOrderInterID,FChkPassItem) '
                            + 'select FBrNo,' + CAST(@pp AS VARCHAR(10))
                            + ',FEntryID,FBatchNo,FNote,
			isnull((select top 1 FItemID from ' + @DBName
			...
			isnull((select top 1 FItemID from ' + @DBName
                            + '.dbo.t_Stock where FNumber=(select FNumber from t_Stock where FItemID=t.FDCStockID)),0),
			isnull((select top 1 FItemID from ' + @DBName
                            + '.dbo.t_Stock where FNumber=(select FNumber from t_Stock where FItemID=t.FSCStockID)),0),
		...
		-----------库存更新----------------------
				...
				from  (select FItemID,FBatchNo,FMTONo,FAuxPropID,FDCStockID as FStockID,FDCSPID as FStockPlaceID,FKFPeriod,FKFDate,SUM(FQty) AS FQty,SUM(FSecQty) AS FSecQty 
						from ' + @DBName
                                    + '.dbo.ICStockBillEntry where  FInterID ='
                                    + CAST(@pp AS VARCHAR(10))
                                    + ' group by FItemID,FBatchNo,FMTONo,FAuxPropID,FDCStockID,FDCSPID,FKFPeriod,FKFDate) u1
				inner join ' + @DBName
                                    + '.dbo.ICInventory t1 on t1.FItemID=u1.FItemID AND t1.FBatchNo=u1.FBatchNo AND t1.FMTONo=u1.FMTONo AND t1.FAuxPropID=u1.FAuxPropID
				   AND t1.FStockID=u1.FStockID AND isnull(t1.FStockPlaceID,0)=isnull(u1.FStockPlaceID,0)  AND t1.FKFPeriod=u1.FKFPeriod AND isnull(t1.FKFDate,'''')=isnull(convert(varchar(10),u1.FKFDate,120),'''')  ';
                                ...
                                SET @Sql = 'insert into ' + @DBName
                                    + '.dbo.ICInventory(FBrNo,FItemID,FBatchNo,FMTONo,FAuxPropID,FStockID,FStockPlaceID,FKFPeriod,FKFDate,FQty,FSecQty)
			...
			----2014-11-13 yexuefeng add AutoIcsale
				...
				from  (select FItemID,FBatchNo,FMTONo,FAuxPropID,FSCStockID as FStockID,FDCSPID as FStockPlaceID,FKFPeriod,FKFDate,SUM(FQty) AS FQty,SUM(FSecQty) AS FSecQty 
						from ' + @DBName
                                            + '.dbo.ICStockBillEntry where  FInterID ='
                                            + CAST(@pp AS VARCHAR(10))
                                            + ' group by FItemID,FBatchNo,FMTONo,FAuxPropID,FSCStockID,FDCSPID,FKFPeriod,FKFDate) u1
				inner join ' + @DBName
                                            + '.dbo.ICInventory t1 on t1.FItemID=u1.FItemID AND t1.FBatchNo=u1.FBatchNo AND t1.FMTONo=u1.FMTONo AND t1.FAuxPropID=u1.FAuxPropID
				   AND t1.FStockID=u1.FStockID AND isnull(t1.FStockPlaceID,0)=isnull(u1.FStockPlaceID,0)  AND t1.FKFPeriod=u1.FKFPeriod AND isnull(t1.FKFDate,'''')=isnull(convert(varchar(10),u1.FKFDate,120),'''')  ';
                                        EXEC(@sql);
				...
				where t1.FItemID is null';
                                ...
                                ELSE
                                    IF @FTranType = 41	--调拨单库存 yexuefeng
                                        BEGIN
                                            SET @Sql = 'EXEC ' + @DBName
                                                + '.dbo.sp_ic41tb_auto_ICInventory  '
                                                + CAST(@pp AS VARCHAR(10));
...
------*********************** 此处要改目标帐套名*****************************************	
                IF EXISTS ( SELECT  1
                            FROM    CON14.AIS_YXDZP2018_3.dbo.ICStockBill
...
------*********************** 此处要改目标帐套名*****************************************
				...
				where t1.FItemID is null';
                        ...
                        ELSE
                            IF @FTranType = 41--调拨单库存 yexuefeng
                                BEGIN
                                    SET @Sql = 'EXEC ' + @DBName
                                        + '.dbo.sp_ic41del_auto_ICInventory  '
                                        + CAST(@FSynchroID AS VARCHAR(10));
		...
		-----------------------------------------
                ...
                SET @Sql = 'update ' + @DBName
                    + '.dbo.ICStockBill set FCheckerID=Null,FStatus=0,FCheckDate=Null  where FInterID='
                    + @FSynchroID;
                EXEC(@sql); 
		
                SET @Sql = 'delete from ' + @DBName
                    + '.dbo.ICStockBill where isnull(FCheckerID,0)=0 and FInterID='
                    + @FSynchroID;
                EXEC(@sql); 
...
--SELECT TOP 1 @FCheckerID=isnull(FProcessUserID,0)@FCheckerID=isnull(FChecker,0),  FROM ICClassMCTaskCenter where FClassTypeID = @FClassTypeID  AND fbillid=@FInterID ORDER BY fid desc
	...
	--生成编号    
                ...
                SET @BNoSql = ' exec ' + @DBName + '.dbo.Ly_GetICBillNo  1,'
                    + @FClassTypeID + ' ,@p2 output';
  ...
  --      set @BNoSql =' exec '+ @DBName+'.dbo.Pro_BosBillNo  ''XSKD'',40020,@p2 output'
		--exec sp_executesql  @BNoSql ,N'@p2   nvarchar(50)   output ',@SBillNo   output
			--t_rp_ARBillOfSH:    FCostType,FFeesMode,FBrID,FExpenseID
                SET @pp = 0;
                SET @nSql = ' exec ' + @DBName
                    + '.dbo.GetICMaxNum ''t_RP_NewReceiveBill'',@p2 output,1,16394';
                EXEC sp_executesql @nSql, N'@p2   int   output ', @pp OUTPUT;
    
                SET @sql = ' insert into ' + @DBName
                    + '.dbo.t_RP_NewReceiveBill(FBillID,FMulCy,FPreAmountFor,FNumber,FDate,FFincDate,FItemClassID,FClassTypeID,
			...
			FReceiveAmountFor,FDiscountAmount,FDiscountAmountFor,FContractID,FRP,FPre,FTranType_CN) '
                    ...
                    + ',FMulCy,FPreAmountFor,' + '''' + @SBillNo + ''''
                    + ',FDate,FFincDate,FItemClassID,FClassTypeID,
			isnull((select top 1 FItemID from ' + @DBName
			...
			isnull((select top 1 FCurrencyID from ' + @DBName
                    + '.dbo.t_Currency where FNumber = (select top 1 FNumber from  t_Currency where FCurrencyID= t.FReceiveCyID)),1),
			isnull((select top 1 FCurrencyID from ' + @DBName
                    + '.dbo.t_Currency where FNumber = (select top 1 FNumber from  t_Currency where FCurrencyID= t.FSettleCyID)),1),
			isnull((select top 1 FUserID from ' + @DBName
		...
		---------插入审核流记录 2014-12-01  yexuefeng
                SET @sql = 'INSERT INTO ' + @DBName
                    + '.dbo.ICClassCheckRecords1000005( FPage ,FBillID , FBillEntryID ,FBillNo ,FBillEntryIndex ,FCheckLevel ,
                   ...
                   FROM ' + @DBName
                    + '.dbo.t_RP_NewReceiveBill WHERE FBillID in ('
                    + CAST(@pp AS VARCHAR(10))
                    + ') UNION
		...
		---------插入审核流记录 2014-12-01  yexuefeng
                SET @sql = ' insert into ' + @DBName
                    + '.dbo.t_rp_Exchange(FIndex,FSerial,FBillID,FExchangeCyID,FExchangeAmountFor,FExchangeExpenseFor,
			FExchangeExpense,FExchangeRate,FExchangeAmount,FSettleCyID,FSettleAmountFor,FSettleAmount) '
                    + ' select FIndex,FSerial,' + CAST(@pp AS VARCHAR(10))
                    + ',FExchangeCyID,FExchangeAmountFor,FExchangeExpenseFor,
			FExchangeExpense,FExchangeRate,FExchangeAmount,FSettleCyID,FSettleAmountFor,FSettleAmount 
			from  t_rp_Exchange a where FBillID=' + @FInterID;
                EXEC(@sql);
	
                SET @sql = ' insert into ' + @DBName
                    + '.dbo.t_rp_ARBillOfSH(FBackAmount_Relative,FBackAmountFor_Relative,FIndex,'
			...
			FLinkCheckAmount,FLinkCheckAmountFor,FLinkCheckQty,FRemainAmount,FRemainAmountFor,'
                    + CAST(@pp AS VARCHAR(10))
                    + ',FClassID_SRC,
			...
			left join (select FInterID,FDetailID,FEntryID from ' + @DBName
                    + '.dbo.ICSaleEntry)t4 on t4.FInterID=t2.FSynchroID and  t4.FEntryID=t3.FEntryID
			where t.FBillID=' + @FInterID;
                EXEC(@sql);	--
		
                SET @sql = 'INSERT INTO ' + @DBName
                    + '.dbo.t_RP_Contact (FBillID,FYear,FPeriod,FRP,FType,FDate,FFincDate,FNumber,FCustomer,FDepartment,FEmployee,FCurrencyID,
			...
			select ' + CAST(@pp AS VARCHAR(10))
                    + ',FYear,FPeriod,FRP,FType,FDate,FFincDate,FNumber,
			isnull((select top 1 FItemID from ' + @DBName
                    + '.dbo.t_Item where FItemClassID=t.FItemClassID and FNumber = (select top 1 FNumber from t_Item where FItemID= t.FCustomer)),0),     
			isnull((select top 1 FItemID from ' + @DBName
                    + '.dbo.t_Department where FNumber =(select top 1 FNumber from t_department where FItemID= t.FDepartment)),0),
...
--**************************************************
                IF EXISTS ( SELECT  1
                            FROM    CON14.AIS_YXDZP2018_3.dbo.t_RP_NewReceiveBill
...
--**************************************************
		  ...
		  --exec(@sql) 
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_RP_Contact where isnull(fbillid,0)>0 and  FBillID='
                    + @FSynchroID;
                EXEC(@sql);
		  
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_rp_ARBillOfSH WHERE isnull(fbillid,0)>0 and  FBillID='
                    + @FSynchroID;
                EXEC(@sql);
		  
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_rp_Exchange where isnull(fbillid,0)>0 and  FBillID='
                    + @FSynchroID;
                EXEC(@sql); 
		  
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_RP_NewReceiveBill where isnull(fbillid,0)>0 and  FBillID='
                    + @FSynchroID;
                EXEC(@sql);
		  
                SET @sql = 'update  t_RP_NewReceiveBill set  FSynchroID=0 where isnull(fbillid,0)>0 and  FBIllID='
                    + @FInterID;
                EXEC(@sql);
...
--11	同步记账凭证  FPosterID 记账人   FPosted 是否过账     	0-未过账,1-已过账                                
    ...
    FROM    ( SELECT    ISNULL(( SELECT TOP 1
                                        Fitemid
                                 FROM   CON14.AIS_YXDZP2018_3.dbo.t_item
                                 WHERE  FItemClassID = 1
                                        AND FNumber = ISNULL(( SELECT TOP 1
                                                              fnumber
                                                              FROM
                                                              t_item
                                                              WHERE
  ...
  --****************************************************************
            ...
            CON14.AIS_YXDZP2018_3.dbo.t_itemdetail b
    WHERE   a.F1 <> b.F1
            AND a.F2 <> b.F2
            AND a.F3 <> b.F3
            AND a.F8 <> b.F8;
                ...
                FETCH NEXT  FROM itemdetail_cursor INTO @F1, @F2, @F3, @F8,
                    @FdetailCount;
                WHILE @@FETCH_STATUS = 0
                    BEGIN
--****************************************************
                        SELECT  @FdetailIDMax = ISNULL(MAX(FdetailID) + 1, 0)
                        FROM    CON14.AIS_YXDZP2018_3.dbo.t_itemdetail;
                        INSERT  INTO CON14.AIS_YXDZP2018_3.dbo.T_itemdetail
                                ( F1 ,
                                  F2 ,
                                  F3 ,
                                  F8 ,
     ...
     --********************************************************
                        ...
                        IF @F8 > 0
                            INSERT  INTO t_itemdetailv
                                    ...
                                    WHERE   FDetailID = @FdetailIDMax;
	--************************************************************* 
                        FETCH NEXT FROM itemdetail_cursor INTO @F1, @F2, @F3,
                            @F8, @FdetailCount; 
                    END;
...
--**************************************
                UPDATE  a
                SET     FNext = FNext + 1
                FROM    CON14.AIS_YXDZP2018_3.dbo.t_Identity a
                WHERE   FName = 't_Voucher';
                SELECT TOP 1
                        @pp = FNext
                FROM    CON14.AIS_YXDZP2018_3.dbo.t_Identity
                WHERE   FName = 't_Voucher';
		--set @nSql=' exec '+ @DBName+'.dbo.GetICMaxNum ''t_Voucher'',@p2 output,1,16394'
		--exec sp_executesql  @nSql ,N'@p2   int   output ',@pp   output
 
                SET @sql = 'update  t_Voucher  set FSynchroID='
                    + CAST(@pp AS VARCHAR)
                    + ',FSynchroIDNum=(select isnull(max(FNumber),0)+1 as FNum from '
                    + @DBName + '.dbo.t_Voucher where FYear='
                    + CAST(@FYear AS VARCHAR) + ' and FPeriod='
                    + CAST(@FPeriod AS VARCHAR) + ')  where FVoucherID='
                    + @FInterID
                    + ' and  ISNULL(FSynchroID,0) not in (select FVoucherID from '
                    + @DBName + '.dbo.t_Voucher  )';
...
,isnull((select top 1 FAccountID from ' + @DBName
                    + '.dbo.t_Account where fNumber=(select top 1 FNumber from t_Account where FAccountID=t.FAccountID)),0)
,isnull((select top 1 FAccountID from ' + @DBName
                    + '.dbo.t_Account where fNumber=(select top 1 FNumber from t_Account where FAccountID=t.FAccountID2)),0)                                                                     
,FAmount,FAmountFor
,isnull((select top 1 FItemID from ' + @DBName
                    + '.dbo.t_organization where fNumber=(select top 1 FNumber from t_organization where fItemID=t.FBrNo )),0)      
,FCashFlowItem,FCurrencyID,FDC,       




 isnull((SELECT top 1 isnull(b.FDetailID,0) FROM   (SELECT ISNULL((SELECT TOP 1 Fitemid FROM  '
                    + @DBName
                    + '.dbo.t_item WHERE FItemClassID=1 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=1 AND FItemID=f1),0)),0) AS F1,
  ISNULL((SELECT TOP 1 Fitemid FROM  ' + @DBName
                    + '.dbo.t_item WHERE FItemClassID=2 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=2 AND FItemID=f2),0)),0) AS F2,
  ISNULL((select TOP 1 Fitemid FROM  ' + @DBName
                    + '.dbo.t_item WHERE FItemClassID=3 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=3 AND FItemID=f3),0)),0) AS F3,
  ISNULL((select TOP 1 Fitemid FROM  ' + @DBName
                    + '.dbo.t_item WHERE FItemClassID=8 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=8 AND FItemID=f8),0)),0) AS F8,
  a.FdetailCount 
   FROM t_itemdetail a WHERE a.FDetailID=t.FDetailID AND a.FDetailID>0 ) a , '
                    + @DBName
                    + '.dbo.t_itemdetail b 
...
,t.FInternalInd,FMeasureUnitID,FQuantity,FResourceID,FSettleNo,FSettleTypeID,FTaskID,FTransNo,FUnitPrice
 from t_VoucherEntry t , t_Voucher ee where t.FVoucherID=ee.FVoucherID and t.FVoucherID='
                    + @FInterID
                    + ' and isnull(ee.FSynchroID,0) not in (select FVoucherID from '
                    + @DBName + '.dbo.t_Voucher where FYear='
                    + CAST(@FYear AS VARCHAR) + ' and FPeriod='
                    + CAST(@FPeriod AS VARCHAR) + ')';
...
--')'
                    'insert into ' + @DBName
                    + '.dbo.t_Voucher(FVoucherID,FAttachments,FBrNo,FCashierID,FChecked,FCheckerID,FCreditTotal,
FDate,FDebitTotal,FEntryCount,FExplanation,FFootNote,FFrameWorkID,FGroupID,FHandler,FInternalInd,FNumber,
FObjectName,FOwnerGroupID,FParameter,FPeriod,FPosted,FPreparerID,FReference,FSerialNum,FTransDate,FTranType,FApproveID,FYear) 
select   ' + CAST(@pp AS VARCHAR(10)) + '
,FAttachments,isnull((select top 1 FItemID from ' + @DBName
                    + '.dbo. t_organization where fNumber=(select top 1 FNumber from t_organization where fItemID=t.FBrNo )),0)      
,-1
,0,-1
,FCreditTotal,FDate,FDebitTotal,FEntryCount,FExplanation,FFootNote,FFrameWorkID,FGroupID
,isnull((select top 1 FItemID from ' + @DBName
                    + '.dbo.t_emp where FNumber = (select top 1 FNumber from t_emp where FItemID= t.FHandler)),null)
,FInternalInd,0
,FObjectName,FOwnerGroupID,FParameter,FPeriod  
,0,isnull((select top 1 FUserID from ' + @DBName
                    + '.dbo.t_User where FName = (select top 1 FName from  t_User where FUserID= t.FPreparerID)),16394)
,FReference,FSerialNum,FTransDate,FTranType,FApproveID,FYear  from t_Voucher t where t.FVoucherID='
                    + @FInterID
                    + ' and isnull(t.FSynchroID,0) not in (select FVoucherID from '
                    + @DBName + '.dbo.t_Voucher where FYear='
                    + CAST(@FYear AS VARCHAR) + ' and FPeriod='
                    + CAST(@FPeriod AS VARCHAR) + ')';
                ...
                SET @sql = 'Update ' + @DBName
                    + '.dbo.t_Voucher set FNumber=(select isnull(max(FNumber),0)+1 as FNum from '
                    + @DBName + '.dbo.t_Voucher where FYear='
                    + CAST(@FYear AS VARCHAR) + ' and FPeriod='
                    + CAST(@FPeriod AS VARCHAR) + ') where FVoucherID='
                    + CAST(@pp AS VARCHAR(10));
        ...
        IF ( LEN(@DBName) > 0 )
            ...
            BEGIN
                SET @sql = 'update  t_Voucher  set FSynchroID=null,FSynchroIDNum=0 where FVoucherID='
                    + @FInterID + ' and FSynchroID in ('
                    + 'select FVoucherID from ' + @DBName
                    + '.dbo.t_Voucher where FChecked <=0 and  FVoucherID='
                    + @FSynchroID + ')';
                EXEC(@Sql);	
		...
		--EXEC(@Sql)	
                ...
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_VoucherEntry where FVoucherID=' + @FSynchroID
                    + ' and FVoucherID in (' + 'select FVoucherID from '
                    + @DBName
                    + '.dbo.t_Voucher where FChecked <=0 and  FVoucherID='
                    + @FSynchroID + ')';
	...
	FExchangeRate,	FCheckAmount,	FCheckAmountFor,	FRemainAmount,	FRemainAMountFor,	FCheckQty,	FRemainQty,
		FAmount,	FAmountFor,	FOldBill,	FCashDiscount,	FContractID,	FContractNo,	FExplanation,	FDelete,	0,	0,
			...
			,	1 as FStatus,	FCheckStatus,	FConnectFlag,	FSource,	FSourceID,	FItemClassID,
				FFincDate,	FCheckDate,	FConfirm,	FAdjustExchangeRate,	FAdjustAmount,	FClassTypeID,	FTaskID,	FResourceID	,FOrderID,
					FBudgetAmountFor,	FOrderAmountFor,		FDC from  t_RP_ARPBill t where  t.FBillID='
                    + @FInterID;
...
--SELECT * FROM t_fielddescription WHERE FTableID =50038 --从表查新单据字段
			...
			FStatus,FToBal,FBillType,FInvoiceType,FItemClassID,FExplanation,FPreparer)
			select ' + CAST(@pp AS VARCHAR(10))
                    + ',FBillID,FYear,FPeriod,FRP,FType,FDate,FFincDate,'
                    + '''' + @SBillNo + '''' + ',
			isnull((select top 1 FItemID from ' + @DBName
                    + '.dbo.t_Item where FItemClassID=t.FItemClassID and FNumber = (select top 1 FNumber from t_Item where FItemID= t.FCustomer)),0),     
			isnull((select top 1 FItemID from ' + @DBName
...
--*********************************************************
                IF EXISTS ( SELECT  1
                            FROM    CON14.AIS_YXDZP2018_3.dbo.t_rp_arpbill
                            WHERE   ISNULL(FChecker, 0) > 0
                                    AND FBillID = @FSynchroID )
                    BEGIN
                ...
                WHERE   FBillID = @FInterID; 
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_RP_Contact WHERE isnull(FRPBillID,0)>0 and  FRPBillID='
                    + @FSynchroID;
                EXEC(@sql);
		  
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_rp_plan_Ar WHERE isnull(fbillid,0)>0 and  FBillID='
                    + @FSynchroID;
                EXEC(@sql);
		  
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_rp_arpbillentry WHERE isnull(fbillid,0)>0 and  FBillID='
                    + @FSynchroID;
                EXEC(@sql); 
		  
                SET @sql = 'delete from ' + @DBName
                    + '.dbo.t_rp_arpbill WHERE isnull(fbillid,0)>0 and  FBillID='
                    + @FSynchroID;
                EXEC(@sql);
		  
                SET @sql = 'update  t_rp_arpbill set  FSynchroID=0 where isnull(fbillid,0)>0 and FBIllID='
                    + @FInterID;
                EXEC(@sql);
