-- 源账套 188.188.1.11(服务器)  AIS_YXYZ  (数据库)
---目账套 188.188.1.4 (服务器)  AIS_YXYZ_3(数据库)
...
begin
	...
	begin
	   ...
	   begin
		...
		set @Sql='insert into ' + @DBName + '.dbo.ICStockBillEntry(FBrNo,FInterID,FEntryID,FBatchNo,FNote,
				...
				FReProduceType,FSourceTranType,FSourceEntryID,FSourceBillNo,FSourceInterId,FOrderEntryID, FOrderBillNo, FOrderInterID,FChkPassItem,FEntrySelfB0455,FEntrySelfA0158) '+
			...
			isnull((select top 1 FItemID from ' + @DBName+'.dbo.t_department where FNumber=(select FNumber from t_department where FItemID=t.FEntrySelfB0455)),0),
			isnull((select top 1 FItemID from ' + @DBName+'.dbo.t_department where FNumber=(select FNumber from t_department where FItemID=t.FEntrySelfA0158)),0)
		...
		----2014-11-13 yexuefeng add AutoIcsale
			...
			BEGIN
			SET @Sql='EXEC ' + @DBName+'.dbo.sp_xsck_save_icsale '+cast(@pp as varchar(10))
	...
	begin
------*********************** 此处要改目标帐套名*****************************************	
	  IF EXISTS(select 1 from CON14.AIS_YXYZ_3.dbo.ICStockBill where isnull(FCheckerID,0)>0 and FInterID= @FSynchroID)
------*********************** 此处要改目标帐套名*****************************************
  begin
...
--SELECT TOP 1 @FCheckerID=isnull(FProcessUserID,0)@FCheckerID=isnull(FChecker,0),  FROM ICClassMCTaskCenter where FClassTypeID = @FClassTypeID  AND fbillid=@FInterID ORDER BY fid desc
  ...
  --      set @BNoSql =' exec '+ @DBName+'.dbo.Pro_BosBillNo  ''XSKD'',40020,@p2 output'
		...
		--以下是插入ICClassCheckStatus1000005 czq2018-02-09	
		
		SET @sql='INSERT INTO ' + @DBName + '.dbo.ICClassCheckStatus1000005
		( FPage ,FBillID , FBillEntryID ,FBillNo ,FBillEntryIndex ,FCurrentLevel ,FCheckMan1 ,FCheckDate1)
		 SELECT  1 FPage ,FBillID ,0 FBillEntryID ,FNumber  FBillNo ,0 FBillEntryIndex , 1 FCurrentkLevel ,
                   FChecker FCheckMan1 ,FCheckDate  FCheckDate1
                   FROM ' + @DBName + '.dbo.t_RP_NewReceiveBill WHERE FBillID in ('+cast(@pp as varchar(10))+') '	
	...
	begin
--**************************************************
IF EXISTS(select 1 from CON14.AIS_YXYZ_3.dbo.t_RP_NewReceiveBill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
--**************************************************
...
DECLARE itemdetail_cursor CURSOR FOR
 --********************************************************
    SELECT a.* FROM   (SELECT ISNULL((SELECT TOP 1 Fitemid FROM CON14.AIS_YXYZ_3.dbo.t_item WHERE  FItemClassID=1 AND  FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE  FItemClassID=1 AND  FItemID=f1),0)),0) AS F1,
  ISNULL((SELECT TOP 1 Fitemid FROM CON14.AIS_YXYZ_3.dbo.t_item WHERE FItemClassID=2 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=2 AND FItemID=f2),0)),0) AS F2,
  ISNULL((select TOP 1 Fitemid FROM CON14.AIS_YXYZ_3.dbo.t_item WHERE FItemClassID=3 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=3 AND FItemID=f3),0)),0) AS F3,
  ISNULL((select TOP 1 Fitemid FROM CON14.AIS_YXYZ_3.dbo.t_item WHERE FItemClassID=8 AND FNumber =ISNULL((SELECT TOP 1 fnumber FROM t_item WHERE FItemClassID=8 AND FItemID=f8),0)),0) AS F8,
  a.FdetailCount 
  --****************************************************************
   FROM t_itemdetail a,t_VoucherEntry b WHERE a.FDetailID=b.FDetailID AND b.FVoucherID=@FinterID AND a.FDetailID>0 ) a ,CON14.AIS_YXYZ_3.dbo.t_itemdetail b 
...
BEGIN
--****************************************************
SELECT @FdetailIDMax=ISNULL(MAX(FdetailID)+1,0) FROM CON14.AIS_YXYZ_3.dbo.t_itemdetail
INSERT INTO CON14.AIS_YXYZ_3.dbo.T_itemdetail (F1,F2,F3,F8,FdetailCount,FdetailID)
...
IF @F1>0
     ...
     --*****************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXYZ_3.dbo.t_Item WHERE  FItemClassID=1 AND  FItemID=@F1),0),ISNULL(@F1,0)  from CON14.AIS_YXYZ_3.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F2>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
     --********************************************************
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXYZ_3.dbo.t_Item WHERE FItemClassID=2 AND FItemID=@F2),0),ISNULL(@F2,0)  from CON14.AIS_YXYZ_3.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F3>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXYZ_3.dbo.t_Item WHERE FItemClassID=3 AND FItemID=@F3),0),ISNULL(@F3,0)  from CON14.AIS_YXYZ_3.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	 IF @F8>0
     insert into t_itemdetailv(fdetailid, fitemclassid, fitemid) 
	 select fdetailid,ISNULL((select TOP 1 fitemclassid FROM CON14.AIS_YXYZ_3.dbo.t_Item WHERE FItemClassID=8 AND FItemID=@F8),0),ISNULL(@F8,0)  from CON14.AIS_YXYZ_3.dbo.t_itemdetail where  FDetailID=@FdetailIDMax
	--************************************************************* 
    FETCH NEXT FROM itemdetail_cursor into @F1,@F2,@F3,@F8,@FdetailCount 
END
...
--**************************************
UPDATE a SET FNext=FNext+1 FROM CON14.AIS_YXYZ_3.dbo.t_Identity a WHERE FName='t_Voucher'
SELECT TOP 1 @pp=FNext FROM CON14.AIS_YXYZ_3.dbo.t_Identity WHERE FName='t_Voucher'
...
--*********************************************************
IF EXISTS(select 1 from CON14.AIS_YXYZ_3.dbo.t_rp_arpbill where isnull(FChecker,0)>0 and FBillID= @FSynchroID)
