alter procedure  pro_HT_EmpGroupSync
as
begin
set nocount on
--1.更新职员类别
--1.1找出（非禁用）职员类别的辅助表分组数据,插入新帐套
...
and t1.FDeleted=0
and not exists 
(select * from t_HT_SyncControl where FOID=t1.FItemID and FType='辅助表-职员类别')
...
select @FItemID=FItemID from t_item where FItemClassID=3 and FNumber=@FNumber
...
Delete from Access_t_Emp where FItemID=@FItemID
...
update t_Item set FName=FName where FItemID=@FItemID and FItemClassID=3
...
from #tmp2 t1
left join 
(select * from t_item where FItemClassID=3 and FDetail=0 and FDeleted=0) t2 on t1.FParentNumber=t2.FNumber
...
where t1.FItemClassID=3 and t1.FDetail=0
...
--exec pro_HT_EmpGroupSync
--delete from t_item where fitemclassid=3 and fdetail=0
--delete from t_HT_SyncControl where FType='辅助表-职员类别'