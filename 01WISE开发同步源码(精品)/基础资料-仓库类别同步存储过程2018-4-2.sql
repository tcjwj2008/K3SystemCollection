alter procedure pro_HT_StockGroupSync

as
begin
set nocount on
--1.更新仓库类别
--1.1找出（非禁用）仓库类别的辅助资料分组数据,插入新帐套
...
and t1.FDeleted=0
and not exists 
(select * from t_HT_SyncControl where FOID=t1.FItemID and FType='辅助表-仓库类别')
...
select @FItemID=FItemID from t_item where FItemClassID=5 and FNumber=@FNumber
...
where (( FUserItemClassRight &  8)=8) and fitemclassid=5 and fuserid<>16394
...
Delete from Access_t_Stock where FItemID=@FItemID
...
update t_Item set FName=FName where FItemID=@FItemID and FItemClassID=5
...
from #tmp2 t1
left join 
(select * from t_item where FItemClassID=5 and FDetail=0 and FDeleted=0) t2 on t1.FParentNumber=t2.FNumber
...
where t1.FItemClassID=5 and t1.FDetail=0
...
--exec pro_HT_StockGroupSync
--delete from t_item where fitemclassid=5 and fdetail=0
--delete from t_HT_SyncControl where FType='辅助表-仓库类别'