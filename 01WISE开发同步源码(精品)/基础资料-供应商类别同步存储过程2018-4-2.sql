alter procedure pro_HT_SupplierGroupSync

as
begin
set nocount on
--1.更新供应商类别
--1.1找出（非禁用）供应商类别的辅助表分组数据,插入新帐套
...
and t1.FDeleted=0
and not exists 
(select * from t_HT_SyncControl where FOID=t1.FItemID and FType='辅助表-供应商类别')
...
select @FItemID=FItemID from t_item where FItemClassID=8 and FNumber=@FNumber
...
Delete from Access_t_Supplier where FItemID=@FItemID
...
update t_Item set FName=FName where FItemID=@FItemID and FItemClassID=8
...
from #tmp2 t1
left join 
(select * from t_item where FItemClassID=8 and FDetail=0 and FDeleted=0) t2 on t1.FParentNumber=t2.FNumber
...
where t1.FItemClassID=8 and t1.FDetail=0
...
--exec pro_HT_SupplierGroupSync
--delete from t_item where fitemclassid=8 and fdetail=0
--delete from t_HT_SyncControl where FType='辅助表-供应商类别'